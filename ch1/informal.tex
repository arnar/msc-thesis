...
by ensuring that a access-control reference monitor is invoked every time
...
accesses to variables are simply logged (often they are already logged by the STM) and only at the end
of the transaction, the logs are inspected by the reference monitor. If any of the logged accesses
...
an exception is raised to invoker of the atomic block.
...
any state changes that happened during the
transaction
...
STM actions, and provided a single function to wrap STM actions into an atomic I/O action.
...
be easily adapted to TMI with minimal changes to code. The TMI monad also encapsulates state, namely the
...
% Þetta hér að ofan er eitt sem væri fínt að sýna í merkingafræðinni.
% Ef þetta er svona súper clear, þá gætum við sýnt það og gert að söluvöru.
% Eins og það er núna, þá er þetta ekkert súper clear fyrir mig, btw.

% Hvar eru orElseTMI og vinir í kóðanum?  Eiga þeir ekki að vera neinstaðar?
